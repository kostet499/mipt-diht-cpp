 ...
 * корректную последовательность. Длина исходной последовательности ≤ 800000.
 */
...
using collections::ListArray;
using collections::Pair;
using collections::Stack;


int read_sequence(ListArray<char> &sequence);
bool solve_bracket_sequence_problem(ListArray<char> &sequence, int &sequence_length);
int main() {
    ListArray<char> sequence = ListArray<char>(800001);
    int sequence_length = read_sequence(sequence);
    if (solve_bracket_sequence_problem(sequence, sequence_length)) {
        for (int index = 0; index < sequence_length; index++) {
            printf("%c", sequence.get(index));
        }
    }
    else {
        puts("IMPOSSIBLE");
...
int read_sequence(ListArray<char> &sequence) {
    ...
    while (true) {
        ...
        if (!(char_ptr == '[' || char_ptr == ']' ||
            ...
            sequence.set(sequence_length++, char_ptr);
        }
    }
    return sequence_length;
}
char get_opposit(char bracket) {
    if (bracket == ']') {
        return '[';
    }
    else if (bracket == '}') {
...
unsigned int get_rest(ListArray<char> &sequence, char* rest, int sequence_length, bool reversed = false) {
    Stack<char> stack = Stack<char>();
    for (int index = 0; index < sequence_length; index++) {
        char bracket = sequence.get(index);
        if (bracket == '}' || bracket == ')' || bracket == ']') {
            if (stack.is_empty()) {
                continue;
            }
            else {
                if (stack.top() == get_opposit(bracket)) {
                    stack.pop();
                }
                else {
                    continue;
    ...
    unsigned int rest_length = stack.length();
    for (int index = 0; index < rest_length; index++) {
        int opposite_bracket = stack.pop();
        rest[index] = !reversed ? get_opposit(opposite_bracket) : opposite_bracket;
    }
    if (reversed) {
        functools::array_reverse(rest, rest_length);
    }
    stack.clear();
    return rest_length;
};
void reverse_sequence(ListArray<char> &sequence, int &sequence_length) {
    sequence.reverse(sequence_length);
    for (int index = 0; index < sequence_length; index++) {
        sequence.set(index, get_opposit(sequence.get(index)));
    }
}
bool sequence_is_correct(ListArray<char> &sequence, int sequence_length) {
    Stack<char> stack = Stack<char>();
    for (int index = 0; index < sequence_length; index++) {
        char char_ptr = sequence.get(index);
        assert(
               char_ptr == '[' || char_ptr == ']' ||
               char_ptr == '(' || char_ptr == ')' ||
               char_ptr == '{' || char_ptr == '}'
        ...
        if (stack.is_empty()) {
            ...
            if (char_ptr == get_opposit(stack.top())) {
                stack.pop();
            }
            else {
                stack.push(char_ptr);
    ...
    bool correct = stack.length() == 0;
    stack.clear();
    return correct;
}
bool solve_bracket_sequence_problem(ListArray<char> &sequence, int &sequence_length) {
    char* right_rest = (char*)malloc(sequence_length * sizeof(char));
    unsigned int right_rest_length = get_rest(sequence, right_rest, sequence_length, false);
    reverse_sequence(sequence, sequence_length);
    char* left_rest = (char*)malloc(sequence_length * sizeof(char));
    unsigned int left_rest_length = get_rest(sequence, left_rest, sequence_length, true);
    reverse_sequence(sequence, sequence_length);
    char* correct_sequence = (char*)malloc(
                                           (left_rest_length + sequence_length + right_rest_length) * sizeof(char)
                                           );
    for (int i = 0; i < left_rest_length; i++){
        correct_sequence[i] = left_rest[i];
    }
    for (int i = 0; i < sequence_length; i++) {
        correct_sequence[i + left_rest_length] = sequence.get(i);
    }
    for (int i = 0; i < right_rest_length; i++){
        correct_sequence[i + left_rest_length + sequence_length] = right_rest[i];
    }
    sequence_length = left_rest_length + sequence_length + right_rest_length;
    sequence.update(sequence_length, correct_sequence);
    if (right_rest_length != 0) {
        free(right_rest);
    }
    if (left_rest_length != 0) {
        free(left_rest);
    }
    return sequence_is_correct(sequence, sequence_length) ? true : false;
}
